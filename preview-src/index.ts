...
window.addEventListener(
  ...
  async (event) => {
    ...
    switch (event.data.type) {
      ...
      case "updateContent": {
        const root = document.querySelector(".nomifactory-ja-preview-body")!;
        ...
        if (event.data.source !== documentResource) {
          root.replaceWith(newContent.querySelector(".nomifactory-ja-preview-body")!);
          ...
          const newRoot = newContent.querySelector(".nomifactory-ja-preview-body")!;
        ...
        window.dispatchEvent(new CustomEvent("vscode.nomifactory-ja-preview.updateContent"));
        break;
      }
    }
  },
